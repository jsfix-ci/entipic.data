/*eslint no-underscore-dangle:0*/
'use strict';

var core = require('entipic.core');
var Text = require('entipic.text');
var _ = core._;
var url = require('url');

exports.normalizeTopic = function(data) {
	data = _.pick(data, 'name', 'uniqueName', 'id', 'type', 'pictureId', 'country', 'pictureDomain', 'englishWikiDescription', 'englishWikiName', 'englishWikiId', 'refIP', 'refHost', 'popularity', 'createdAt');

	data._id = data.id;
	delete data.id;

	if (data.englishWikiDescription) {
		if (data.englishWikiDescription.length > 400) {
			data.englishWikiDescription = core.text.wrapAt(data.englishWikiDescription, 400);
		} else if (data.englishWikiDescription.length < 50) {
			delete data.englishWikiDescription;
		}
	}

	data = core.util.clearObject(data);
	return data;
};

exports.normalizeUniqueName = function(data) {
	data = _.pick(data, 'name', 'uniqueName', 'topicId', 'pictureId', 'country', 'lang', 'wikiId', 'wikiLang', 'isLocal', 'status', 'popularity', 'createdAt');

	data._id = core.util.md5(data.uniqueName.toLowerCase());

	data = core.util.clearObject(data);
	return data;
};

exports.normalizePicture = function(data) {
	data = _.pick(data, 'sourceUrl', 'sourceDomain', 'dHash', 'topicId', 'status', 'createdAt');
	data._id = exports.pictureId(data);
	data.sourceDomain = data.sourceDomain || url.parse(data.sourceUrl).host;

	data = core.util.clearObject(data);
	return data;
};

exports.normalizeUnknownName = function(data) {
	data = _.pick(data, 'name', 'country', 'lang', 'ip', 'host', 'createdAt');
	data.name = data.name.trim();
	data.uniqueName = Text.uniqueName(data.name);
	if (data.lang) {
		data.uniqueName = Text.cultureUniqueName(data.uniqueName, data.lang, data.country);
	}
	data._id = core.util.md5(data.uniqueName);

	data = core.util.clearObject(data);
	return data;
};

exports.uniqueNameId = function(un) {
	return core.util.md5(un.toLowerCase());
};

exports.pictureId = function(picture) {
	return core.util.md5([picture.topicId, picture.dHash].join('-'));
};
